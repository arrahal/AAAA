...
import { Users, AlertCircle, ArrowLeft, CheckCircle2, UserPlus, Building2, UserCog, PlusCircle } from 'lucide-react';
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-extrabold text-slate-800">
            لوحة القيادة — {MONTHS_AR[db.currentMonth]} {new Date().getFullYear()}
          </h2>
          <p className="text-xs text-slate-500 font-medium">متابعة الأداء وإحصائيات التأطير التربوي</p>
        </div>
      ...
      <div className="bg-gradient-to-br from-blue-50 via-indigo-50/70 to-blue-100/60 border border-blue-200/80 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-blue-800 font-medium leading-relaxed mt-1">
            {sup.province} · {sup.region} — تأطير ومواكبة {db.ecoles.length} مدرسة · {db.animateurs.length} أستاذ · {db.groupes.length} فوج
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-2xl p-4 text-center shadow-md">
          ...
          <span className="text-xs text-blue-100 font-bold mt-1.5 block">إجمالي التلاميذ</span>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-2xl p-4 text-center shadow-md">
          ...
          <span className="text-xs text-emerald-100 font-bold mt-1.5 block">إجمالي الإناث</span>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 text-white rounded-2xl p-4 text-center shadow-md">
          ...
          <span className="text-xs text-amber-100 font-bold mt-1.5 block">غيابات الشهر</span>
        </div>
        ...
        <div className="bg-gradient-to-br from-rose-500 to-red-600 text-white rounded-2xl p-4 text-center shadow-md">
          ...
          <span className="text-xs text-rose-100 font-bold mt-1.5 block">نسبة الغياب العامة</span>
        </div>
      </div>
      ...
      {isEmpty && (
        <div className="bg-white border-2 border-dashed border-blue-200 rounded-2xl p-6 text-center space-y-3 shadow-sm">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800">التطبيق فارغ ومستعد لاستقبال بياناتك التربوية!</h3>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
              الأساتذة — تقدم الزيارات الشهري
            </h3>
            <button
              onClick={() => onNavigateTab('animateurs')}
              className="text-xs text-blue-600 hover:text-blue-800 font-bold flex items-center gap-1 cursor-pointer"
            >
              <span>عرض الكل ({db.animateurs.length})</span>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 font-medium">
                        {a.zone} · {gs.length} أفواج · {eff} تلميذ
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-center">
                        ...
                        <span className="text-[10px] text-slate-400 block leading-tight">زيارة</span>
                      </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                الأفواج الأكثر غياباً هذا الشهر
              </h3>
            </div>
            <button
          ...
          {topAbsentGroups.length === 0 ? (
            <div className="text-center py-6 text-slate-400 text-xs">
              ...
              <p>لا توجد حالات غياب مرتفعة مسجلة هذا الشهر</p>
            </div>
          ) : (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500">
                        الأستاذ: {anim ? anim.nom.split(' ')[0] : '—'} · {g.eff} تلميذ · مستوى: {g.niveauReel || g.niveau}
                      </div>
                    </div>
