  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-24">
      ...
      {selectedAnim ? (
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">
                  {selectedAnim.zone} · {animGroups.length} أفواج مؤطرة
                </p>
              </div>
            ...
            {usedDays.length === 0 || allTimes.length === 0 ? (
              <div className="text-center py-8 text-slate-400 text-xs">
                ...
                <p>لا يوجد جدول أوقات جديد مسجل لهذا الأستاذ بعد</p>
                <button
                  onClick={() => onOpenAddSlotModal(selectedAnim.id)}
                  className="mt-2 text-blue-600 font-bold hover:underline"
                >
                  اضغط هنا لإضافة أوقات الحصص
                </button>
              </div>
            ) : (
                ...
                <table className="w-full text-center border-collapse text-xs">
                  <thead>
                    <tr className="bg-blue-700 text-white font-bold">
                      <th className="p-2 border border-blue-800 font-bold min-w-[70px]">الوقت</th>
                      {usedDays.map((d) => (
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 font-medium mt-0.5">
                    {g.niveauReel || g.niveau} · {g.eff} تلميذ
                  </div>

                  <div className="flex flex-wrap gap-1 mt-2">
                    {Object.entries(g.horaires || {}).length === 0 ? (
                      <span className="text-[10px] text-slate-400 font-medium">لا توجد حصص مبرمجة</span>
                    ) : (
        ...
        <div className="text-center py-8 text-slate-400">لا يوجد أساتذة في النظام</div>
      )}
    </div>
  );
};
