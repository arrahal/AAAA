...
export const VisitsTab: React.FC<VisitsTabProps> = ({ db, onUpdateDb, onOpenVisitModal }) => {
  ...
  const filteredVisits = visitReports.filter((r) => {
    ...
    const matchesSearch =
      (anim && anim.nom.includes(searchTerm)) ||
      r.text.includes(searchTerm) ||
  ...
  const handleDeleteVisit = (visitId: number) => {
    if (confirm('🗑️ هل أنت تأكد من حذف تقرير هذه الزيارة؟')) {
      onUpdateDb((prev) => ({
        ...prev,
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
            ...
            <span>قسم الزيارات التفقدية والمواكبة</span>
          </h2>
          <p className="text-xs text-slate-500 font-medium">سجل الزيارات الصفية والتقييمات البيداغوجية</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-2xl p-3 shadow-sm">
          ...
          <span className="text-[10px] text-emerald-100 font-bold mt-1 block">إجمالي الزيارات</span>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-2xl p-3 shadow-sm">
          ...
          <span className="text-[10px] text-blue-100 font-bold mt-1 block">أساتذة تم إشرافهم</span>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-indigo-600 text-white rounded-2xl p-3 shadow-sm">
          ...
          <span className="text-[10px] text-purple-100 font-bold mt-1 block">مجموع المؤطرين</span>
        </div>
      </div>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="بحث باسم الأستاذ، التقرير، التاريخ..."
      ...
      {filteredVisits.length === 0 ? (
        <div className="bg-white rounded-2xl p-8 text-center border border-slate-200 space-y-3 shadow-sm">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800">لا توجد زيارات مسجلة تطابق البحث</h3>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-slate-800">{anim ? anim.nom : 'أستاذ'}</span>
                    ...
                    <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                      {groupe ? `${groupe.ecole} — الفوج ${groupe.groupe}` : 'مؤسسة تعليمية'}
                    </p>
                  </div>
                ...
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-xs text-slate-700 leading-relaxed font-medium">
                  <div className="font-bold text-slate-900 mb-1 flex items-center gap-1 text-[11px]">
                    ...
                    <span>ملاحظات وتوجيهات المشرف التربوي:</span>
                  </div>
                  {v.text}
                </div>
                ...
                <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                  ...
                  >
                    <span>تعديل التقييم والزيارة</span>
                  </button>
