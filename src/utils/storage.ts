  ...
  if (snap) {
    if (snap.groupes) {
      ...
      updatedGroupes = data.groupes.map((g) => {
        ...
        if (sg) {
          return { ...g, absences: sg.absences, visits: sg.visits };
        }
        return g;
      });
    }
    if (snap.animScores) {
      ...
      updatedAnimateurs = data.animateurs.map((a) => {
        ...
        if (sa) {
          return { ...a, scores: JSON.parse(JSON.stringify(sa.scores || {})) };
        }
        return a;
      });
    }
    ...
    updatedAnimateurs = data.animateurs.map((a) => ({ ...a, scores: {} }));
  }
